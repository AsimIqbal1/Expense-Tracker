    ...
    return (
        <Flex my={theme.space.x2} p={theme.space.x4} border={`1px solid ${theme.colors.border[colorMode]}`} borderRadius={theme.borderRadius.md} flexDir="column" gap={theme.space.x4} >
            {walletTransactions.map(transaction => (
                <Flex border={`1px solid ${theme.colors.border[colorMode]}`} color={theme.colors.text[colorMode]} p={theme.space.x3} borderRadius={theme.borderRadius.md} justifyContent={"space-between"} align="center" _hover={{ bg: theme.colors.border[colorMode], cursor: 'pointer' }} onClick={showTransactionDetails(transaction)}>
